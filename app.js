...
veryOldInventors  = inventors.filter((name) => {
  return name.year > 1500 && name.year < 1600;
});
...
sortedByBirthYear  = inventors.sort((date1, date2) => {
...
totalYearsLived = inventors.reduce((acc, life) => {
    return acc + (life.passed - life.year);
}
, 0);
...
// Complete the exercise in the space below:
travelMethodCounts =travelMethods.reduce((acc, trr) => {
    if(acc[trr]){
        acc[trr] = acc[trr] + 1;
    } 
    else {
